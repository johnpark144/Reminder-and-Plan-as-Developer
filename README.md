...
- mytube 프로젝트 CI/CD 하기 (썸네일 화질 high로 하고 없는 것만 디폴트로 다시 바꾸기, 다크모드시 X창 색 변경, X창 위치 조정, Delete all history버튼 패딩 조정//  로컬에서 에러가 심해서 다시 만들기, next/font 빌트인 되있는 라이브러리로 변경, Home UI 예쁘게 변경, 페이지 폴더와 컴포넌트 폴더 분리)
