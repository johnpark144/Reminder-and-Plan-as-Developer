...
###### 4) doortodoor 프로젝트
■ 이름을 그렇게 지은이유 : doortodoor라는 뜻은 집집마다 돌아다닌다는 의미가 있는데, 사이트의 용도와 맞기도 하였고
로고에대해서 좋은 아이디어가 생각나서 지정하게 되었다

■ doortodoor 어려웠던점: (그림을 포함하자)
1. 지도맵 라이브러리와 지리에 대한 Api를 처음배워 사용하는데, 위도, 경도에 대한 정보를
파이어베이스 데이터베이스에 저장하는데 어떤식으로 저장할것이며 어떤식으로 다시 불러올지 고민을 엄청하게 되었다,
하루걸려 알게된건 위도,경도를 위한 데이터 저장형식이 따로 있다는걸 그제서야 알게되어 고민을 해결 할 수있었다.
2. 위도,경도 저장시 클릭할때마다 데이터를 저장하는건 너무빈번해서 효율적이지 못하다는것을 구현도중에 알게되었는데,
배열형식으로 저장할시 어떤식으로 전체데이터를 불러와서, 위도,경도를 표시한사람이 입력한 디테일정보들을 또 어떻게 구현할까
애먹게 되었는데, 사용자와 위도,경도 정보를 가진 전체 배열을 하나의 배열로 만들어 화면에 map을 이용해 꾸리고,
그 위도,경도를 입력한 사용자만 디테일정보를 입력하고 수정하게끔 만들었다
